...
var app = new Vue({
    ...
    data: {
        ...
        dinamic_objs: [
            ...
            {uFc: ''}, {um3p: ''}, {uSubsidy: ''}, 
            {uSubtotal: ''}, { uTotal: ''}//these values will be accessed as v-text from the index.html file
        ],
        status: '', //The status attribute will hide the report and bill divs and is also linked to the close function
        isActive: 1,//It will activate or deactivate the add, report and update buttons
    ...
    methods: {
        ...
        add(){
            if (this.values[4].iname.length < 0 || 
                ...
                alert('The Name, Stratum and Consumption fields cannot be empty, please check the fields');
            }else {
                ...
                this.list.push({//unshift to add it at the start of the list
                    ...
                    subtotal: this.values[4].isubtotal,
                    total: this.values[4].itotal,
                });
            }
        },
        deleteItem(index){
        ...
        update(item){
            this.isActive = 0;
            this.values[4].iname = item.name;
            this.values[4].istratum = item.stratum;
            this.values[4].iconsume = item.consumption;
        },
        save(index){//duplicated id errors here
            this.add();
            this.list.splice(index, 1);
            this.values[4].iname = '';
            this.values[4].istratum = '';
            this.values[4].iconsume = '';
            this.isActive = 1;
        }
    }
